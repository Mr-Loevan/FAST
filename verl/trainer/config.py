...
class DataConfig:
    train_files: str = ""
    val_files: Any = ""  # Support single string or list of strings, use 'Any' to avoid Union issues with OmegaConf
    val_names: Optional[List[str]] = None  # Optional names for each validation set
    ...
    def post_init(self):
        # Convert val_files to list if it's a single string
        if isinstance(self.val_files, str):
            self.val_files = [self.val_files] if self.val_files else []
        
        # Generate default val_names if not provided
        if self.val_names is None and self.val_files:
            self.val_names = [f"val_set_{i}" for i in range(len(self.val_files))]
        elif self.val_names and len(self.val_names) != len(self.val_files):
            print(f"Warning: val_names length ({len(self.val_names)}) does not match val_files length ({len(self.val_files)}). Using default names.")
            self.val_names = [f"val_set_{i}" for i in range(len(self.val_files))]
...
class AlgorithmConfig:
    ...
    """kl coefficient"""
    kl_type: str = "fixed"
    """kl controller type, support `fixed`, `adaptive`, `accuracy_based`, `group_accuracy_based`"""
    kl_horizon: float = 10000.0
    """kl horizon for adaptive kl controller"""
    kl_target: float = 0.1
    """target kl for adaptive kl controller"""
    kl_min_coef: float = 1e-4
    """minimum kl coefficient for accuracy-based controller (β_min in formula)"""
    kl_max_coef: float = 0.1
    """maximum kl coefficient for accuracy-based controller (β_max in formula)"""
    ...
    dynamic_filter_schedule: Optional[List[dict]] = None
    """List of filter schedule configs, each with 'epoch_ratio' and filter values
    Example: [
        {"epoch_ratio": 0.5, "filter_low": 0.3, "filter_high": 0.99},
        {"epoch_ratio": 1.0, "filter_low": 0.01, "filter_high": 0.7}
    ]
    """
