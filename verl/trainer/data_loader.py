...
def create_dataloader(config: DataConfig, tokenizer: PreTrainedTokenizer, processor: Optional[ProcessorMixin]):
    ...
    # Create validation dataloaders - now support multiple validation sets
    val_dataloaders = {}
    
    if not config.val_files:
        print("Warning: No validation files specified.")
        return train_dataloader, {}
    
    for idx, val_file in enumerate(config.val_files):
        if not val_file:  # Skip empty strings
            continue
        ...
        val_name = config.val_names[idx] if config.val_names else f"val_set_{idx}"
        val_dataloaders[val_name] = val_dataloader
        
        assert len(val_dataloader) >= 1
        print(f"Size of val dataloader '{val_name}': {len(val_dataloader)}")
